...
const Analytics = () => {
      ...
      try {
        ...
        if (tasksRes.ok) {
          ...
          tasksData.forEach((task) => {
            if (!loadedTasks[task.date]) {
              loadedTasks[task.date] = [];
            }
            loadedTasks[task.date].push({
              id: task.id,
              text: task.text,
              status: task.status,
            });
  ...
  const addTask = async () => {
    ...
    setTasks((prev) => ({
      ...
      [dateKey]: [
        ...(prev[dateKey] || []),
        { id: tempId, text: textVal, status: "Upcoming" },
      ],
    }));
    setNewTask("");
    try {
      ...
      if (response.ok) {
        ...
        // Replace temp ID with the real database ID
        setTasks((prev) => {
          const updated = [...(prev[dateKey] || [])];
          const idx = updated.findIndex((t) => t.id === tempId);
          if (idx !== -1) {
            updated[idx] = { ...updated[idx], id: createdTask.id };
          }
        ...
        setTasks((prev) => {
          const updated = [...(prev[dateKey] || [])].filter((t) => t.id !== tempId);
          return { ...prev, [dateKey]: updated };
        });
      }
    } catch (err) {
  ...
  const updateTaskStatus = async (index, status) => {
    ...
    try {
      ...
      const response = await fetch(`/api/calendar-tasks/${taskToUpdate.id}`, {
        method: "PUT",
        headers,
      ...
      if (!response.ok) {
        ...
        setTasks((prev) => {
          const updated = [...prev[selectedDate]];
          const idx = updated.findIndex((t) => t.id === taskToUpdate.id);
          if (idx !== -1) {
            updated[idx] = { ...updated[idx], status: previousStatus };
          }
          return { ...prev, [selectedDate]: updated };
        });
      }
      ...
      <main className="p-4 md:p-6 lg:p-8">
        ...
        <div className="flex gap-2 mb-6 border-b border-[#CCCCCC] dark:border-gray-700">
          <button
            ...
            className={`px-4 py-2 font-medium transition-colors relative ${activeTab === "courses"
          ...
          <button
            ...
            className={`px-4 py-2 font-medium transition-colors relative ${activeTab === "calendar"
              ? "text-[#ff6d34] dark:text-[#ff6d34]"
              : "text-[#2D3436]/60 hover:text-[#2D3436] dark:text-gray-400"
              }`}
          >
                    ...
                    {filteredCourses.length > 0 ? (
                      filteredCourses.map((course) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 text-xs rounded-full font-medium ${course.level === 'Beginner'
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex flex-col">
                            <span className={`text-sm font-semibold ${isToday && dayInfo.isCurrentMonth
                              ? 'text-[#ff6d34] dark:text-[#ff6d34]'
                              : dayInfo.isCurrentMonth
